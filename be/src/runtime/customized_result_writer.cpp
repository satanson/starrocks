...
Status CustomizedResultWriter::init(RuntimeState* state) {
    LOG(WARNING) << "invoke Customized::Init";
...
StatusOr<TFetchDataResultPtrs> CustomizedResultWriter::process_chunk(Chunk* chunk) {
    auto num_rows = chunk == nullptr ? 0 : chunk->num_rows();
    LOG(WARNING) << "invoke Customized::process_chunk: num_rows=" << num_rows;
    SCOPED_TIMER(_total_timer);
...
static std::string to_hex_string(std::string s) {
    std::string hex_str;
    hex_str.reserve(s.size() * 2);
    const char* alphabet = "0123456789abcdef";
    for (auto a : s) {
        hex_str.push_back(alphabet[(((uint8_t)a) >> 4) & 0xf]);
        hex_str.push_back(alphabet[((uint8_t)a) & 0xf]);
    }
    return hex_str;
}
StatusOr<TFetchDataResultPtr> CustomizedResultWriter::_process_chunk(Chunk* chunk) {
    ...
    for (auto i = 0; i < num_rows; ++i) {
        ...
        std::string str_row;
        LOG(WARNING) << "Customized serialize BEGIN: packed_data=" << row.packed_data.size();
        RETURN_IF_ERROR(compact_serializer.serialize(&row, &str_row));
        LOG(WARNING) << "Customized serialize BEGIN: row_size=" << str_row.size()
                     << ", hex_str=" << to_hex_string(str_row);
