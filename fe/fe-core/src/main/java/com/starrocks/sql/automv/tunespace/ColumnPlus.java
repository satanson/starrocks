...
public final class ColumnPlus {
    ...
    public static ColumnPlus fieldToColumn(Field field) {
        ...
        column.setName(ObjectUtils.isEmpty(desc.name()) ? field.getName() : desc.name());
        Type type = Objects.requireNonNull(NAME_TO_TYPE_MAP.getOrDefault(desc.type(), null));
        column.setType(TypePlus.of(type, desc.len(), desc.precision(), desc.scale()).getType());
        column.setIsAutoIncrement(desc.autoIncrement());
        column.setIsAllowNull(desc.nullable());
        column.setComment(desc.comment());
        column.setIsKey(desc.isKey() || desc.isPartitionColumn() || desc.isBucketColumn());
    ...
    public static <T> T unpack(Class<T> klass, List<ColumnPlus> columns, TRowFormat row) {
        ...
        try {
            ...
            for (int i = 0; i < columns.size(); ++i) {
                ColumnPlus column = columns.get(i);
