    ...
    protected void enumerate() {
        ...
        for (Map.Entry<Pair<OptExpression, OptExpression>, Set<Pair<ColumnRefOperator, ColumnRefOperator>>>
            ...
            Set<Pair<ColumnRefOperator, ColumnRefOperator>> inversePairs = pairs.stream().map(Pair::inverse).collect(
                    Collectors.toSet());
            List<CardinalityPreservingJoinTableGardener.CardinalityPreservingBiRel> biRels = Lists.newArrayList();
            biRels.addAll(CardinalityPreservingJoinTableGardener.getCardPreservings(lhsOptExpr, rhsOptExpr, true));
            biRels.addAll(CardinalityPreservingJoinTableGardener.getCardPreservings(rhsOptExpr, lhsOptExpr, false));
            for (CardinalityPreservingJoinTableGardener.CardinalityPreservingBiRel biRel : biRels) {
