...
public class TablePlus {
    ...
    public String getDeleteSql(List<Op> conjuncts) {
        Preconditions.checkArgument(!conjuncts.isEmpty());
        ...
        PrettyPrinter printer = new PrettyPrinter();
        printer.addItems("DELETE FROM ")
                .add(table.getName()).add(" WHERE ")
                .addItemsWithNlDel("AND ", conditions);
        return printer.getResult();
    }
    ...
    public <T> String getInsertSql(Collection<T> objects) {
        ...
        printer.indentEnclose(() -> {
            printer.addSuperStepsWithNl(",", tuples);
        });
        return printer.getResult();
    }
